...
# into lightweight pipe-delimited index files in agent/context/.
#
...
if [[ -d "$CONTEXT_DIR" ]]; then
    ...
    mkdir -p "$CONTEXT_DIR"
    msg "Created agent/context/"
fi

msg "Generating index files from agent/xml_parsed/..."
...
# ---------------------------------------------------------------------------
{
    ...
    find "$XML_PARSED_DIR/tables" -name '*.xml' -type f | sort | while IFS= read -r file; do
...
} > "$CONTEXT_DIR/fields.index"

field_lines=$(( $(wc -l < "$CONTEXT_DIR/fields.index") - 1 ))
...
# ---------------------------------------------------------------------------
{
    ...
    find "$XML_PARSED_DIR/relationships" -name '*.xml' -type f | sort | while IFS= read -r file; do
...
} > "$CONTEXT_DIR/relationships.index"

rel_lines=$(( $(wc -l < "$CONTEXT_DIR/relationships.index") - 1 ))
...
# ---------------------------------------------------------------------------
{
    ...
    find "$XML_PARSED_DIR/layouts" -name '*.xml' -type f | sort | while IFS= read -r file; do
...
} > "$CONTEXT_DIR/layouts.index"

layout_lines=$(( $(wc -l < "$CONTEXT_DIR/layouts.index") - 1 ))
...
# ---------------------------------------------------------------------------
{
    ...
    find "$XML_PARSED_DIR/script_stubs" -name '*.xml' -type f | sort | while IFS= read -r file; do
...
} > "$CONTEXT_DIR/scripts.index"

script_lines=$(( $(wc -l < "$CONTEXT_DIR/scripts.index") - 1 ))
...
# ---------------------------------------------------------------------------
{
    ...
    find "$XML_PARSED_DIR/table_occurrences" -name '*.xml' -type f | sort | while IFS= read -r file; do
...
} > "$CONTEXT_DIR/table_occurrences.index"

to_lines=$(( $(wc -l < "$CONTEXT_DIR/table_occurrences.index") - 1 ))
...
# ---------------------------------------------------------------------------
{
    ...
    find "$XML_PARSED_DIR/value_lists" -name '*.xml' -type f | sort | while IFS= read -r file; do
...
} > "$CONTEXT_DIR/value_lists.index"

vl_lines=$(( $(wc -l < "$CONTEXT_DIR/value_lists.index") - 1 ))
msg "  value_lists.index: $vl_lines value lists"

# ---------------------------------------------------------------------------
# Report results
# ---------------------------------------------------------------------------
total_lines=$(( field_lines + rel_lines + layout_lines + script_lines + to_lines + vl_lines ))
total_size=$(du -sh "$CONTEXT_DIR" 2>/dev/null | cut -f1)

echo ""
msg "Done!"
msg "  Output: agent/context/ ($total_lines total entries, ${total_size})"
msg "  Files: fields.index, relationships.index, layouts.index, scripts.index, table_occurrences.index, value_lists.index"

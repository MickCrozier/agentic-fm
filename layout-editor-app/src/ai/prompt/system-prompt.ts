...
// Returns the static portion of the system prompt — instructions, docs, fields.
// This never includes layout state so it can be prompt-cached by Anthropic.
export function buildStaticSystemPrompt(opts: {
  customInstructions?: string;
  availableFields?: AvailableField[];
}): string {
  const { customInstructions, availableFields } = opts;

  let prompt = buildCoreInstructions();
...
// Returns the current layout state as a context message to inject into conversation.
// Kept separate so the static system prompt can be cached.
export function buildLayoutStateMessage(opts: {
  layoutName?: string;
  state?: LayoutState | null;
}): string | null {
  const { layoutName, state } = opts;
  if (!state || (state.objects.length === 0 && state.parts.length === 0)) return null;

  let msg = `## Current layout${layoutName ? `: ${layoutName}` : ''}`;
...
// Kept for backwards compatibility — assembles full prompt in one block (no caching).
...
function buildCoreInstructions(): string {
  return `You are a FileMaker layout design assistant embedded in a web-based visual layout editor. You can both advise on layout design AND directly apply changes to the layout.
...
[
  ...
  { "op": "add", "object": { "type": "portal", "x": 10, "y": 100, "width": 600, "height": 200, "fmName": "InvoiceItem", "children": [
    { "type": "field", "x": 0, "y": 0, "width": 200, "height": 22, "fmName": "Description", "fieldRef": "InvoiceItem::Description" },
    { "type": "field", "x": 200, "y": 0, "width": 80, "height": 22, "fmName": "Quantity", "fieldRef": "InvoiceItem::Quantity" }
  ]} }
]
\`\`\`
...
- **portal**: Use \`"fmName"\` for the related table occurrence name. Include \`"children"\` array with field columns; child field x-coordinates are relative to the portal left edge, y is the row y-offset (first row is always y=0). Portal column header labels go above the portal as separate text objects, not inside it.
...
- \`themeClass\`: a theme style class name from the Theme Classes list in this prompt. Do NOT use internal type names like "edit_box", "text_box", "button", "line" — only use names from the provided list. Set to null to remove the theme class.

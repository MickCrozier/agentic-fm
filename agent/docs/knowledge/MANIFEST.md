...
| `field-definitions.md` | fmxmlsnippet format for field definitions (XMFD clipboard class): Normal fields, Calculated fields (unstored), auto-enter calc fields. Critical gotcha: calculated field formula must be inline CDATA in `<Calculation table="TO">` — NOT inside a `<Text>` child or `<TableOccurrenceReference>`. Includes SaXML vs fmxmlsnippet diff table. | field definition, XMFD, calculated field, fmxmlsnippet, field snippet, Manage Database, paste fields, fieldType Calculated, storeCalculationResults, auto-enter calculation, clipboard field, field format, field XML |
